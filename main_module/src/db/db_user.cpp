...
crow::json::wvalue DB::getUserDataProfile(const std::string& userId, bool includeCourses, bool includeTests, bool includeGrades) {
    ensureConnection();
    ...
    if (includeCourses) {
        ...
        if (PQresultStatus(res) == PGRES_TUPLES_OK) {
            ...
            std::cerr << "Get user courses failed: " << PQerrorMessage(conn) << std::endl;
        }
        PQclear(res);
    }
